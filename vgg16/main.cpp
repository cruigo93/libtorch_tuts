...
int main() {
    ...
    auto net = std::make_shared<VGG16>();

	// Create multi-threaded data loader for MNIST data
	// Make sure to enter absolute path to the data directory for no errors later on
	auto data_loader = torch::data::make_data_loader<torch::data::samplers::SequentialSampler>(
			std::move(torch::data::datasets::MNIST("/home/cruigo/Documents/cpp/libtorch_tuts/vgg16/mnist").map(torch::data::transforms::Normalize<>(0.13707, 0.3081)).map(
